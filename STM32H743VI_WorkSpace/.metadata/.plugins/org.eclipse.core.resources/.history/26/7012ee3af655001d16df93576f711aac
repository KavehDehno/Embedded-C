...
int main(void)
{
  ...
  // Blue LED blinks 5 times with T=100ms
    int Start_Counter = 0;
	...
	if (HAL_FDCAN_ActivateNotification(&hfdcan1, FDCAN_IT_TX_COMPLETE |
			FDCAN_IT_RX_FIFO0_FULL | FDCAN_IT_RX_FIFO0_NEW_MESSAGE |
			FDCAN_IT_RX_FIFO1_FULL | FDCAN_IT_RX_FIFO1_NEW_MESSAGE |
			FDCAN_IT_RX_BUFFER_NEW_MESSAGE, FDCAN_IT_TX_COMPLETE) != HAL_OK)
	{
		Error_Handler();
	}

	if (HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_TX_COMPLETE |
			FDCAN_IT_RX_FIFO0_FULL | FDCAN_IT_RX_FIFO0_NEW_MESSAGE |
			FDCAN_IT_RX_FIFO1_FULL | FDCAN_IT_RX_FIFO1_NEW_MESSAGE |
			FDCAN_IT_RX_BUFFER_NEW_MESSAGE, FDCAN_IT_TX_COMPLETE) != HAL_OK)
	{
...
//	FifoFillLevel = HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1);
//	USART_Send((char*)FifoFillLevel);
//	sprintf(usr_msg,"\n");
//	USART_Send(usr_msg);
...
//	uint32_t FifoFillLevel;
//	FifoFillLevel = HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan2);
//	USART_Send((char*)FifoFillLevel);
//	sprintf(usr_msg,"\n");
//	USART_Send(usr_msg);
...
{
	sprintf(usr_msg,"HAL_FDCAN_TxEventFifoCallback\r\n");
	USART_Send(usr_msg);
}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
	...
	else if( RxFifo0ITs == FDCAN_IT_RX_FIFO0_NEW_MESSAGE)
	{
		if(hfdcan->Instance == FDCAN1)
		{
			...
			sprintf(usr_msg,"fdcan1 : ");
			USART_Send(usr_msg);

			USART_Send((char*)Received_msg);
			USART_Send("\n");

			// Send message to be transmitted to CAN 2
			FDCAN2_Tx();
		}
		else
		{
...
void HAL_FDCAN_ErrorStatusCallback(FDCAN_HandleTypeDef *hfdcan, uint32_t ErrorStatusITs)
{
	...
	sprintf(usr_msg,"HAL_FDCAN_ErrorStatusCallback\r\n");
	USART_Send(usr_msg);
}
...
void HAL_FDCAN_TxBufferCompleteCallback(FDCAN_HandleTypeDef *hfdcan, uint32_t BufferIndexes)
{
	sprintf(usr_msg,"HAL_FDCAN_TxBufferCompleteCallback\r\n");
	USART_Send(usr_msg);
}
